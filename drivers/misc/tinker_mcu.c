...
int tinker_mcu_screen_power_up(int dsi_id)
{
	...
	for (i = 0; i < 100; i++) {
		 send_cmds(g_mcu_data[dsi_id]->client, REG_PORTB);

		ret = recv_cmds(g_mcu_data[dsi_id]->client, recv_buf, 1);
	...
	ret = send_cmds(g_mcu_data[dsi_id]->client, "8104");
...
int tinker_mcu_set_bright(int bright, int dsi_id)
{
	...
	ret = i2c_master_send(g_mcu_data[dsi_id]->client, cmd, 2);
...
int tinker_mcu_get_brightness(int dsi_id)
{
...
int tinker_mcu_is_connected(int dsi_id)
{
			...
			const struct i2c_device_id *id)
{
	...
	printk("find panel mcu at i2c-%d\n", i2c_id);
